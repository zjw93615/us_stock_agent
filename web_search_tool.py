...
class WebSearchAndAnalysisTool:
    """
    网络搜索和信息整理工具
...
# 在tools.py中的集成工具类
class WebSearchIntegrationTool:
    """
    集成到现有工具系统中的Web搜索工具
    """
    def __init__(self):
        self.name = "search_web_info"
        self.description = "搜索网络信息并进行AI分析，类似DeepSeek或豆包的网络搜索功能"
        self.parameters = {
            "query": {"type": "str", "description": "搜索查询关键词"},
            "search_type": {"type": "str", "description": "搜索类型：'general', 'news', 'finance', 'company', 'academic'"},
            "max_results": {"type": "int", "description": "最大结果数量，默认10"},
            "analysis_focus": {"type": "str", "description": "分析重点：'investment_risk', 'market_trend', 'company_analysis', 'general'等"}
        }
    ...
    def run(self, query: str, search_type: str = "general", max_results: int = 10, analysis_focus: str = "general") -> Dict:
        ...
        try:
            result = self.search_tool.search_and_analyze(
                query=query,
                search_type=search_type,
