...
def visualization():
    ...
    if not ticker and query:
        ...
        common_stocks = ['AAPL', 'GOOGL', 'MSFT', 'AMZN', 'TSLA', 'META', 'NVDA', 'NFLX', 'BABA', 'JD']
        ...
        if chart_type == 'technical':
            ...
            chart_data = {
                ...
                'datasets': [
                    {
                        'label': '收盘价',
                        'data': [serializable_result['Close'].get(date) for date in dates],
                        'borderColor': 'rgba(75, 192, 192, 1)',
                    ...
                    {
                        'label': '交易量',
                        'data': [serializable_result['Volume'].get(date) for date in dates],
                        'type': 'bar',
...
def stream():
    ...
    def generate():
        ...
        try:
            ...
            def run_analysis():
                try:
                    stream_agent = LLMStockAgent(news_api_key=None, model_name="qwen-flash")
                    result = stream_agent.analyze(user_query, max_steps=5, step_callback=step_callback)
                    # 分析完成，发送结束信号
                    data_queue.put(None)
                except Exception as e:
                    # 发送错误信息
